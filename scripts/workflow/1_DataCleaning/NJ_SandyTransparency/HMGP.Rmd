...
table(HMGP_Ocean$Disbursed.Year) #missing year on 2 projects. Ignore for now.
...
sankey_nodes2A <- list(
  HMGP_Ocean %>% select(7) %>% unique,          #Node 1 #Project / program / bucket
  HMGP_Ocean %>% select(9) %>% unique) %>%      #Node 2 #Municipality
  unlist(use.names = F) %>% 
  unique()
# 
...
DisAmtHMGPOcean <- subset(link_2A, names_tgt=="COUNTYWIDE")
DisAmtHMGPTR <- subset(link_2A, names_tgt=="TOMS RIVER TOWNSHIP")
DisAmtHMGPTR$Federal.Disbursed.Amount.Adj.PerHU <- DisAmtHMGPTR$Federal.Disbursed.Amount.Adj / 9651
DisAmtHMGPTR$Federal.Disbursed.Amount.Adj.PerMile <- DisAmtHMGPTR$Federal.Disbursed.Amount.Adj / 134

LocalShareHMGPOcean <- subset(link_2B, names_tgt=="COUNTYWIDE")
LocalShareHMGPTR <- subset(link_2B, names_tgt=="TOMS RIVER TOWNSHIP")
LocalShareHMGPTR$Local.Share.Adj.PerHU <- LocalShareHMGPTR$Local.Share.Adj / 9651
LocalShareHMGPTR$Local.Share.Adj.PerHUPerMile <- LocalShareHMGPTR$Local.Share.Adj / 134
