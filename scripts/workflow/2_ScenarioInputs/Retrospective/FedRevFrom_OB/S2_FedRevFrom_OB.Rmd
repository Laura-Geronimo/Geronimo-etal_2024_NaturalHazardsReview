---
title: "S2_FedRevFrom_OB"
author: "Laura Geronimo"
date: "2024-09-13"
output: html_document
---

The purpose of this worksheet is to estimate the NPV for benefits to the Federal Government for Scenario 2. 

Simplifying assumption:
-No revenues from premiums after 2012 because all homes have been relocated
...
#Applying S2 Simplifying Assumption: No revenue after 2012
```{r include = FALSE}
S2_FedRevOB <- FedRevOB

S2_FedRevOB$OB_NFIP_PolicyCost_adj[S2_FedRevOB$Year > 2012] <- NA
...
amount_cols <-  grep("^OB_", colnames(S2_FedRevOB), value = TRUE)

# Apply the discount function
S2_FedRevOB_dis2pct <- dis2pct(S2_FedRevOB, amount_cols, year_index_col = "YearIndex")

# Remove undiscounted columns
S2_FedRevOB_dis2pct <- S2_FedRevOB_dis2pct %>%
  select(Year, YearIndex, ends_with("_dis2pct"))

###Summing to NPV ####
#Create a new data frame that sums the discounted columns for NPV (2020USD)
S2_FedRevOB_dis2pct_NPV <- S2_FedRevOB_dis2pct %>%
  summarise(across(ends_with("_dis2pct"), sum, na.rm = TRUE))

#pivoting tables longer
S2_FedRevOB_dis2pct_NPV <- S2_FedRevOB_dis2pct_NPV %>%
  pivot_longer(
...
# Apply the discount function
S2_FedRevOB_dis3pct <- dis3pct(S2_FedRevOB, amount_cols, year_index_col = "YearIndex")

# Remove undiscounted columns
S2_FedRevOB_dis3pct <- S2_FedRevOB_dis3pct %>%
  select(Year, YearIndex, ends_with("_dis3pct"))

###Summing to NPV ####
#Create a new data frame that sums the discounted columns for NPV (2020USD)
S2_FedRevOB_dis3pct_NPV <- S2_FedRevOB_dis3pct %>%
  summarise(across(ends_with("_dis3pct"), sum, na.rm = TRUE))

#pivoting tables longer
S2_FedRevOB_dis3pct_NPV <- S2_FedRevOB_dis3pct_NPV %>%
  pivot_longer(
    cols = everything(),      # Pivot all columns
    names_to = "variable",     # New column with variable names
...
# Apply the discount function
S2_FedRevOB_dis7pct <- dis7pct(S2_FedRevOB, amount_cols, year_index_col = "YearIndex")

# Remove undiscounted columns
S2_FedRevOB_dis7pct <- S2_FedRevOB_dis7pct %>%
  select(Year, YearIndex, ends_with("_dis7pct"))

###Summing to NPV ####
#Create a new data frame that sums the discounted columns for NPV (7070USD)
S2_FedRevOB_dis7pct_NPV <- S2_FedRevOB_dis7pct %>%
  summarise(across(ends_with("_dis7pct"), sum, na.rm = TRUE))

#pivoting tables longer
S2_FedRevOB_dis7pct_NPV <- S2_FedRevOB_dis7pct_NPV %>%
  pivot_longer(
    cols = everything(),      # Pivot all columns
    names_to = "variable",     # New column with variable names
...
```{r include = FALSE}
#pivoting tables longer
S2_FedRevOB_NPV <- S2_FedRevOB_dis2pct_NPV %>%
  left_join(S2_FedRevOB_dis3pct_NPV, by = "variable") %>%
  left_join(S2_FedRevOB_dis7pct_NPV, by = "variable")
...
write.csv(S2_FedRevOB_NPV, file.path(path, "S2_FedRevOB_NPV.csv"), row.names = TRUE)

```

#QC
```{r include= FALSE}
S2_FedRevOB_dis2pct_NPV <- sum(S2_FedRevOB_NPV$dis2pct)
S2_FedRevOB_dis3pct_NPV <- sum(S2_FedRevOB_NPV$dis3pct)
S2_FedRevOB_dis7pct_NPV <- sum(S2_FedRevOB_NPV$dis7pct)

S2_FedRevOB_dis2pct_NPV
S2_FedRevOB_dis3pct_NPV
S2_FedRevOB_dis7pct_NPV
```