---
title: "S4_MuniCostFomOB_Table"
author: "Laura Geronimo"
date: "2024-08-29"
output: html_document
---
The purpose of this worksheet is to estimate the NPV for costs to Toms River for Scenario 4. We discount the costs and group them into buckets for analysis.

We discount at 2%, 3%, 6%, and 7% based on justifications provided in the SI.

Assumptions:
-Public works for OB line drops by 40% after 2012, proportional to local road network removed
-Public safety line for OB drops by 50% after 2012, proportional to housing units relocated (proxy for people relocated)
-School district line for OB drops by 50% after 2012, proportional to housing units relocated (proxy for school children relocated)
...
-Municipal cost share for buyouts = 25% of market price for 920 bought out homes
...
# Reducing public works expenditures post 2012 by 40% (proportional to road network removed). Retaining 60%
# Reducing public safety, and school expenditures post 2012 by 50% (proportional to housing units removed). Retaining 50%
```{r include = FALSE}

S4_MuniCostOB <- MuniCostOB
S4_MuniCostOB$OB_Public_Works_adj[S4_MuniCostOB$Year > 2012] <- 
  S4_MuniCostOB$OB_Public_Works_adj[S4_MuniCostOB$Year > 2012] * 0.60
S4_MuniCostOB$OB_Public_Safety_adj[S4_MuniCostOB$Year > 2012] <- 
  S4_MuniCostOB$OB_Public_Safety_adj[S4_MuniCostOB$Year > 2012] * 0.50
S4_MuniCostOB$OB_SchoolExpend_adj[S4_MuniCostOB$Year > 2012] <- 
  S4_MuniCostOB$OB_SchoolExpend_adj[S4_MuniCostOB$Year > 2012] * 0.50

```

# Calculating municipal cost share for Buyouts (25 % of market price for 920 homes)
```{r include= FALSE}
...
OB_LocalShare_BuyoutCost_adj <- OB_S4_2011_SP_Tot_adj * 0.25

#Adding buyout costs to data frame 
S4_MuniCostOB$OB_LocalShare_BuyoutCost_adj[S4_MuniCostOB$Year==2012] <- OB_LocalShare_BuyoutCost_adj
```
...
amount_cols <-  grep("^OB_", colnames(S4_MuniCostOB), value = TRUE)

# Apply the discount function
S4_MuniCostOB_dis2pct <- dis2pct(S4_MuniCostOB, amount_cols, year_index_col = "YearIndex")

# Remove undiscounted columns
S4_MuniCostOB_dis2pct <- S4_MuniCostOB_dis2pct %>%
  select(Year, YearIndex, ends_with("_dis2pct"))

###Summing to NPV ####
#Create a new data frame that sums the discounted columns for NPV (2020USD)
S4_MuniCostOB_dis2pct_NPV <- S4_MuniCostOB_dis2pct %>%
  summarise(across(ends_with("_dis2pct"), sum, na.rm = TRUE))

###creating buckets ####
#Summing Muni Share for Response and Recovery
S4_MuniCostOB_dis2pct_NPV<- S4_MuniCostOB_dis2pct_NPV %>%
...
# Apply the discount function
S4_MuniCostOB_dis3pct <- dis3pct(S4_MuniCostOB, amount_cols, year_index_col = "YearIndex")

# Remove undiscounted columns
S4_MuniCostOB_dis3pct <- S4_MuniCostOB_dis3pct %>%
  select(Year, YearIndex, ends_with("_dis3pct"))

###Summing to NPV ####
#Create a new data frame that sums the discounted columns for NPV (3030USD)
S4_MuniCostOB_dis3pct_NPV <- S4_MuniCostOB_dis3pct %>%
  summarise(across(ends_with("_dis3pct"), sum, na.rm = TRUE))

###creating buckets ####
#Summing Muni Share for Response and Recovery
S4_MuniCostOB_dis3pct_NPV<- S4_MuniCostOB_dis3pct_NPV %>%
...
# Apply the discount function
S4_MuniCostOB_dis6pct <- dis6pct(S4_MuniCostOB, amount_cols, year_index_col = "YearIndex")

# Remove undiscounted columns
S4_MuniCostOB_dis6pct <- S4_MuniCostOB_dis6pct %>%
  select(Year, YearIndex, ends_with("_dis6pct"))

###Summing to NPV ####
#Create a new data frame that sums the discounted columns for NPV (6060USD)
S4_MuniCostOB_dis6pct_NPV <- S4_MuniCostOB_dis6pct %>%
  summarise(across(ends_with("_dis6pct"), sum, na.rm = TRUE))

###creating buckets ####
#Summing Muni Share for Response and Recovery
S4_MuniCostOB_dis6pct_NPV<- S4_MuniCostOB_dis6pct_NPV %>%
...
# Apply the discount function
S4_MuniCostOB_dis7pct <- dis7pct(S4_MuniCostOB, amount_cols, year_index_col = "YearIndex")

# Remove undiscounted columns
S4_MuniCostOB_dis7pct <- S4_MuniCostOB_dis7pct %>%
  select(Year, YearIndex, ends_with("_dis7pct"))

###Summing to NPV ####
#Create a new data frame that sums the discounted columns for NPV (7070USD)
S4_MuniCostOB_dis7pct_NPV <- S4_MuniCostOB_dis7pct %>%
  summarise(across(ends_with("_dis7pct"), sum, na.rm = TRUE))

###creating buckets ####
#Summing Muni Share for Response and Recovery
S4_MuniCostOB_dis7pct_NPV<- S4_MuniCostOB_dis7pct_NPV %>%
...
```{r include = FALSE}
S4_MuniCostOB_NPV <- S4_MuniCostOB_dis2pct_NPV %>%
  left_join(S4_MuniCostOB_dis3pct_NPV, by = "variable") %>%
  left_join(S4_MuniCostOB_dis6pct_NPV, by = "variable") %>%
  left_join(S4_MuniCostOB_dis7pct_NPV, by = "variable")
...
write.csv(S4_MuniCostOB_NPV, file.path(path, "S4_MuniCostOB_NPV.csv"), row.names = TRUE)

```

#QC
```{r include= FALSE}
S4_MuniCostOB_dis2pct_NPV <- sum(S4_MuniCostOB_NPV$dis2pct)
S4_MuniCostOB_dis3pct_NPV <- sum(S4_MuniCostOB_NPV$dis3pct)
S4_MuniCostOB_dis6pct_NPV <- sum(S4_MuniCostOB_NPV$dis6pct)
S4_MuniCostOB_dis7pct_NPV <- sum(S4_MuniCostOB_NPV$dis7pct)

S4_MuniCostOB_dis2pct_NPV
S4_MuniCostOB_dis3pct_NPV
S4_MuniCostOB_dis6pct_NPV
S4_MuniCostOB_dis7pct_NPV
```