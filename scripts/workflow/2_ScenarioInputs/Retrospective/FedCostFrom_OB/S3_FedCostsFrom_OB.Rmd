---
title: "S3_FedCostsFrom_OB"
author: "Laura Geronimo"
date: "2024-09-13"
output: html_document
---

The purpose of this worksheet is to estimate the NPV for costs to the Federal Gov for Scenario 3. We discount the costs and group them into buckets for analysis.
...
-Federal cost share for Response & Recovery direct to Household (23% less proportional to homes bought out- redirected)
-Federal NFIP claims (less amount redirected to buyouts for year 2012, assume the rest are sticky)
-Federal cost share for beach nourishment (assume doubles)

-Federal cost share for buyouts = 75% of market price for 421 bought out homes
...
#for S3 NFIP damages and claims estimates
OB_DmgClaims_est <- read.csv('./data/BaselineData/OB_SandyDmgsAndClaimsEstByScenario/Est_Dmg_Claims_Sandy_byScenario.csv')

```

#Adjusting S1 inputs based on S3 assumptions

## Reducing Federal cost share for Response & Recovery direct to Household by 23% (proportional to homes bought out- redirected). Retaining 77%
```{r include=FALSE}
S3_FedCostOB <- FedCostOB

S3_FedCostOB$OB_FedShare_DCA_Housing_LandLordIncentive_LMI <- S3_FedCostOB$OB_FedShare_DCA_Housing_LandLordIncentive_LMI*0.77

S3_FedCostOB$OB_FedShare_DCA_Housing_NEP_LMI <- S3_FedCostOB$OB_FedShare_DCA_Housing_NEP_LMI *0.77

S3_FedCostOB$OB_FedShare_DCA_LMI_HomeownerRebuilding <- S3_FedCostOB$OB_FedShare_DCA_LMI_HomeownerRebuilding*0.77

S3_FedCostOB$OB_FedShare_DCA_Resettlement_LMI <- S3_FedCostOB$OB_FedShare_DCA_Resettlement_LMI*0.77

S3_FedCostOB$OB_FedShare_DCA_Resettlement_UN <- S3_FedCostOB$OB_FedShare_DCA_Resettlement_UN*0.77

S3_FedCostOB$OB_FedShare_DCA_Resettlement_UN <- S3_FedCostOB$OB_FedShare_DCA_Resettlement_UN*0.77

S3_FedCostOB$OB_FedShare_DCA_RREM_LMI <- S3_FedCostOB$OB_FedShare_DCA_RREM_LMI*0.77

S3_FedCostOB$OB_FedShare_DCA_RREM_UN<- S3_FedCostOB$OB_FedShare_DCA_RREM_UN*0.77

S3_FedCostOB$OB_FedShare_IA <- S3_FedCostOB$OB_FedShare_IA *0.77


```


#RFederal NFIP claims (less amount redirected to buyouts for year 2012, assume the rest are sticky)
```{r include=FALSE}
#obtaining NFIP claims estimated for S3 cluster
S3_EstNFIPClaims_2012 <- OB_DmgClaims_est$S3[OB_DmgClaims_est$X=="Est_NFIP_Claims"]

#adjusting estimate to 2020 USD
S3_EstNFIPClaims_2012_adj <- S3_EstNFIPClaims_2012 * Adjust$CPI_Multiplier_USD2020[Adjust$Year==2012]

#Creating bucket with original NFIP claims
S3_FedCostOB<- S3_FedCostOB %>%
...
S3_FedCostOB$OB_NFIP_Claims_adj[S3_FedCostOB$Year == 2012] <- 
  S3_FedCostOB$OB_NFIP_Claims_adj[S3_FedCostOB$Year == 2012] - S3_EstNFIPClaims_2012_adj


```

## Doubling beach nourishment costs
```{r include = FALSE}
S3_FedCostOB$OB_FedShare_BeachNourish_adj <- S3_FedCostOB$OB_FedShare_BeachNourish_adj*2
```
...
OB_FedShare_BuyoutCost_adj <- OB_S3_2011_SP_Tot_adj * 0.75

#Adding buyout costs to data frame 
S3_FedCostOB$OB_FedShare_BuyoutCost_adj[S3_FedCostOB$Year==2012] <- OB_FedShare_BuyoutCost_adj
```
...
amount_cols <-  grep("^OB_", colnames(S3_FedCostOB), value = TRUE)

# Apply the discount function
S3_FedCostOB_dis2pct <- dis2pct(S3_FedCostOB, amount_cols, year_index_col = "YearIndex")

# Remove undiscounted columns
S3_FedCostOB_dis2pct <- S3_FedCostOB_dis2pct %>%
  select(Year, YearIndex, ends_with("_dis2pct"))

###Summing to NPV ####
#Create a new data frame that sums the discounted columns for NPV (2020USD)
S3_FedCostOB_dis2pct_NPV <- S3_FedCostOB_dis2pct %>%
...
S3_FedCostOB_dis2pct_NPV <- S3_FedCostOB_dis2pct_NPV %>%
...
# Apply the discount function
S3_FedCostOB_dis3pct <- dis3pct(S3_FedCostOB, amount_cols, year_index_col = "YearIndex")

# Remove undiscounted columns
S3_FedCostOB_dis3pct <- S3_FedCostOB_dis3pct %>%
  select(Year, YearIndex, ends_with("_dis3pct"))

###Summing to NPV ####
#Create a new data frame that sums the discounted columns for NPV (3030USD)
S3_FedCostOB_dis3pct_NPV <- S3_FedCostOB_dis3pct %>%
  summarise(across(ends_with("_dis3pct"), sum, na.rm = TRUE))

###creating buckets ####
#Summing Fed Share for Response and Recovery Direct to Municipal Government / School District
S3_FedCostOB_dis3pct_NPV <- S3_FedCostOB_dis3pct_NPV %>%
...
# Apply the discount function
S3_FedCostOB_dis7pct <- dis7pct(S3_FedCostOB, amount_cols, year_index_col = "YearIndex")

# Remove undiscounted columns
S3_FedCostOB_dis7pct <- S3_FedCostOB_dis7pct %>%
  select(Year, YearIndex, ends_with("_dis7pct"))

###Summing to NPV ####
#Create a new data frame that sums the discounted columns for NPV (7070USD)
S3_FedCostOB_dis7pct_NPV <- S3_FedCostOB_dis7pct %>%
  summarise(across(ends_with("_dis7pct"), sum, na.rm = TRUE))

###creating buckets ####
#Summing Fed Share for Response and Recovery Direct to Municipal Government / School District
S3_FedCostOB_dis7pct_NPV <- S3_FedCostOB_dis7pct_NPV %>%
...
```{r include = FALSE}
#pivoting tables longer
S3_FedCostOB_NPV <- S3_FedCostOB_dis2pct_NPV %>%
  left_join(S3_FedCostOB_dis3pct_NPV, by = "variable") %>%
  left_join(S3_FedCostOB_dis7pct_NPV, by = "variable")
...
write.csv(S3_FedCostOB_NPV, file.path(path, "S3_FedCostOB_NPV.csv"), row.names = TRUE)

```

#QC
```{r include= FALSE}
S3_FedCostOB_dis2pct_NPV <- sum(S3_FedCostOB_NPV$dis2pct)
S3_FedCostOB_dis3pct_NPV <- sum(S3_FedCostOB_NPV$dis3pct)
S3_FedCostOB_dis7pct_NPV <- sum(S3_FedCostOB_NPV$dis7pct)

S3_FedCostOB_dis2pct_NPV
S3_FedCostOB_dis3pct_NPV
S3_FedCostOB_dis7pct_NPV
```