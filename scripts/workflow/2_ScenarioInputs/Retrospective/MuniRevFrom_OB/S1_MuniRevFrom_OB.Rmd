...
---
The purpose of this worksheet is to estimate the NPV for benefits to Toms River for Scenario 1. We discount the benefits and group them into buckets for analysis.
...
S1_MuniRevOB_dis2pct_NPV <- MuniRevOB_dis2pct_NPV %>%
  pivot_longer(
...
# Apply the discount function
MuniRevOB_dis3pct <- dis3pct(MuniRevOB, amount_cols, year_index_col = "YearIndex")

# Remove undiscounted columns
MuniRevOB_dis3pct <- MuniRevOB_dis3pct %>%
  select(Year, YearIndex, ends_with("_dis3pct"))

###Summing to NPV ####
#Create a new data frame that sums the discounted columns for NPV (3030USD)
MuniRevOB_dis3pct_NPV <- MuniRevOB_dis3pct %>%
  summarise(across(ends_with("_dis3pct"), sum, na.rm = TRUE))

###creating buckets ####
#Summing Federal Flows for Response and Recovery
MuniRevOB_dis3pct_NPV<- MuniRevOB_dis3pct_NPV %>%
...
#pivoting tables longer
S1_MuniRevOB_dis3pct_NPV <- MuniRevOB_dis3pct_NPV %>%
  pivot_longer(
    cols = everything(),      # Pivot all columns
    names_to = "variable",     # New column with variable names
...
# Apply the discount function
MuniRevOB_dis6pct <- dis6pct(MuniRevOB, amount_cols, year_index_col = "YearIndex")

# Remove undiscounted columns
MuniRevOB_dis6pct <- MuniRevOB_dis6pct %>%
  select(Year, YearIndex, ends_with("_dis6pct"))

###Summing to NPV ####
#Create a new data frame that sums the discounted columns for NPV (6060USD)
MuniRevOB_dis6pct_NPV <- MuniRevOB_dis6pct %>%
  summarise(across(ends_with("_dis6pct"), sum, na.rm = TRUE))

###creating buckets ####
#Summing Federal Flows for Response and Recovery
MuniRevOB_dis6pct_NPV<- MuniRevOB_dis6pct_NPV %>%
...
#pivoting tables longer
S1_MuniRevOB_dis6pct_NPV <- MuniRevOB_dis6pct_NPV %>%
  pivot_longer(
    cols = everything(),      # Pivot all columns
    names_to = "variable",     # New column with variable names
...
# Apply the discount function
MuniRevOB_dis7pct <- dis7pct(MuniRevOB, amount_cols, year_index_col = "YearIndex")

# Remove undiscounted columns
MuniRevOB_dis7pct <- MuniRevOB_dis7pct %>%
  select(Year, YearIndex, ends_with("_dis7pct"))

###Summing to NPV ####
#Create a new data frame that sums the discounted columns for NPV (7070USD)
MuniRevOB_dis7pct_NPV <- MuniRevOB_dis7pct %>%
  summarise(across(ends_with("_dis7pct"), sum, na.rm = TRUE))

###creating buckets ####
#Summing Federal Flows for Response and Recovery
MuniRevOB_dis7pct_NPV<- MuniRevOB_dis7pct_NPV %>%
...
#pivoting tables longer
S1_MuniRevOB_dis7pct_NPV <- MuniRevOB_dis7pct_NPV %>%
  pivot_longer(
    cols = everything(),      # Pivot all columns
    names_to = "variable",     # New column with variable names
...
```{r include = FALSE}
#pivoting tables longer
S1_MuniRevOB_NPV <- S1_MuniRevOB_dis2pct_NPV %>%
  left_join(S1_MuniRevOB_dis3pct_NPV, by = "variable") %>%
  left_join(S1_MuniRevOB_dis6pct_NPV, by = "variable") %>%
  left_join(S1_MuniRevOB_dis7pct_NPV, by = "variable")
...
write.csv(S1_MuniRevOB_NPV, file.path(path, "S1_MuniRevOB_NPV.csv"), row.names = TRUE)

```

#QC
```{r include= FALSE}
S1_MuniRevOB_dis2pct_NPV <- sum(S1_MuniRevOB_NPV$dis2pct)
S1_MuniRevOB_dis3pct_NPV <- sum(S1_MuniRevOB_NPV$dis3pct)
S1_MuniRevOB_dis6pct_NPV <- sum(S1_MuniRevOB_NPV$dis6pct)
S1_MuniRevOB_dis7pct_NPV <- sum(S1_MuniRevOB_NPV$dis7pct)

S1_MuniRevOB_dis2pct_NPV
S1_MuniRevOB_dis3pct_NPV
S1_MuniRevOB_dis6pct_NPV
S1_MuniRevOB_dis7pct_NPV
```